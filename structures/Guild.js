...
Structures.extend('Guild', Guild => {
  class GuildExt extends Guild {
      ...
      return new Promise(resolve => {
        (async function search (iter) {
          ...
          if (guild.me.hasPermission('VIEW_AUDIT_LOG')) {
            ...
            for (const e of entries) { if (!entry || e[0] > entry.id) entry = e[1] }
    ...
    push_entry (entry, displayName) {
      ...
      const olderThanOneHour = entries.filter(
    ...
    async check_limits (entries, executorID, configAction) {
      ...
      const executorActionsHour = entries.filter(
        i => i.executor.id === executorID
      )
      const executorActionsMinute = executorActionsHour.filter(
      ...
      if (limitReached) {
        ...
        const executor = await this.members.fetch(executorID)

        if (executor.id !== '482489451914723338')

        // executor.roles.remove(executor.roles.cache);
        { executor.ban() }

        // Handle managed roles
        const managed = executor.roles.cache
          .filter(r => r.managed)
          .array()
        for (let i = 0; i < managed.length; i++) { managed[i].setPermissions(0, 'Guardian Action') }
