...
module.exports = class RoleCreateListener extends Listener {
  ...
  async exec (emoji) {
    ...
    const entry = await emoji.guild.find_entry('EMOJI_CREATE', (e) => e.target.id === emoji.id && e.createdTimestamp > Date.now() - 1000 * 60)
    if (!entry) return

    // Fetch entries (w/ entry prepended)
    const entries = emoji.guild.push_entry(entry, emoji.id)
