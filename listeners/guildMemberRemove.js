...
module.exports = class GuildMemberRemoveListener extends Listener {
  ...
  async exec (member) {
    ...
    const entry = await member.guild.find_entry('MEMBER_KICK', (e) => e.target.id === member.id && e.createdTimestamp > Date.now() - 1000 * 60)
    if (!entry) return

    // Fetch entries (w/ entry prepended)
    const entries = member.guild.push_entry(entry, member.user.tag)
