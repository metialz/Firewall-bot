...
module.exports = class ChannelDeleteListener extends Listener {
  ...
  async exec (channel) {
    ...
    const entry = await channel.guild.find_entry('CHANNEL_DELETE', (e) => e.target.id === channel.id && e.createdTimestamp > Date.now() - 1000 * 60)
    if (!entry) return

    // Fetch entries (w/ entry prepended)
    const entries = channel.guild.push_entry(entry, `#${channel.name}`)
