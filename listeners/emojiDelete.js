...
module.exports = class RoleDeleteListener extends Listener {
  ...
  async exec (emoji) {
    ...
    const entry = await emoji.guild.find_entry('EMOJI_DELETE', (e) => e.target.id === emoji.id && e.deletedTimestamp > Date.now() - 1000 * 60)
    if (!entry) return

    // Fetch entries (w/ entry prepended)
    const entries = emoji.guild.push_entry(entry, emoji.name)
