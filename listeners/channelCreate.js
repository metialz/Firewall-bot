...
module.exports = class ChannelCreateListener extends Listener {
  ...
  async exec (channel) {
    ...
    const entry = await channel.guild.find_entry('CHANNEL_CREATE', (e) => e.target.id === channel.id && e.createdTimestamp > Date.now() - 1000 * 60)
    if (!entry) return

    // Fetch entries (w/ entry prepended)
    const entries = channel.guild.push_entry(entry, `${channel.id}`)
